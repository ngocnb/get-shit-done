...
## Dimension 7.5: API Dependency Detection

**Question:** Do frontend tasks depend on backend APIs that exist or are created in the same phase?

**Critical check to prevent:** Frontend calling non-existent API endpoints.

**Process:**
1. Scan all frontend tasks for API calls (`fetch('/api/...')`, `useGetXxxQuery`, axios calls)
2. Extract endpoint paths from task actions
3. For each endpoint:
   - Check if backend task creates this endpoint (same phase, lower wave)
   - OR search codebase to verify endpoint already exists
4. Flag any frontend task calling an API with no backend provider

**Red flags:**
- Frontend task calls `/api/models/alibaba` but no backend task creates this endpoint
- Query hook references endpoint not registered in main.py/routes
- Frontend wave precedes backend wave for same API

**Example — missing API:**
```yaml
issue:
  dimension: api_dependency
  severity: blocker
  description: "Frontend task calls /api/models/alibaba but no backend endpoint exists or is created"
  plan: "03"
  task: 1
  frontend_call: "useGetAlibabaModelsQuery() → /api/models/alibaba"
  backend_status: "NOT FOUND"
  fix_hint: "Add backend task to create /api/models/alibaba endpoint in Wave 1, make frontend task depend on it (Wave 2)"
```

**Example — wave ordering:**
```yaml
issue:
  dimension: api_dependency
  severity: blocker
  description: "Frontend task in Wave 1 calls API created in Wave 2"
  plan: "02"
  task: 3
  frontend_wave: 1
  backend_task: "Plan 03 Task 1"
  backend_wave: 2
  fix_hint: "Move frontend task to Wave 3, or move backend task to Wave 1"
```

**Skip if:** Phase has no frontend tasks, or no API calls detected.
